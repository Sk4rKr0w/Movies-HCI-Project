...
const forgotPassword = async (req, res) => {
  ...
  const info        = await transporter.sendMail({
    ...
    html: `
      <p>Hai richiesto di resettare la password. Il link è valido per 1h:</p>
      <a href="${resetLink}">${resetLink}</a>
    `,
  });

  console.log("Preview URL ethereal:", require("nodemailer").getTestMessageUrl(info));
  return res.status(200).json({
    message:    "Email di reset inviata (in dev su Ethereal)",
